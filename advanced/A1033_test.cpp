...
int main(void)
{
	...
	int next_st = 0;
	int pass_node = 0;
	//整体策略为：首先保证邮箱中的油可以跑到下一个站，如果加满也不能到达下一站，那么结束，输出跑的距离
	//          其次，如果油价比下下一站的贵，那么不加
	//                 如果比下一站的便宜，那就加油，要么加到可以到达更便宜的站点，要么加满。
	//注意passed_dis的更新；
	...
	for (int i = 0; i < n + 1; ++i)
	{
		cout << "station " << i << " " << stations[i].distance << endl;
	}

	for (int i = 0; i<n; ++i)
	{
		cout << "station" << i << "'s adding is" << adding[i] << endl;
